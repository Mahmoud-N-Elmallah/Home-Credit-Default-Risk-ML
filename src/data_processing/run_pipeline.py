...
def load_data(raw_paths):
    ...
    # Force schema for problematic columns to prevent String inference
    schema_overrides = {
        'SK_ID_CURR': pl.Int64,
        'SK_ID_BUREAU': pl.Int64,
        'SK_ID_PREV': pl.Int64,
        'AMT_INCOME_TOTAL': pl.Float64,
        'AMT_CREDIT': pl.Float64,
        'AMT_ANNUITY': pl.Float64,
        'AMT_GOODS_PRICE': pl.Float64,
        'AMT_PAYMENT': pl.Float64,
        'AMT_INSTALMENT': pl.Float64,
        'DAYS_ENTRY_PAYMENT': pl.Float64,
        'DAYS_INSTALMENT': pl.Float64,
        'AMT_BALANCE': pl.Float64,
        'AMT_CREDIT_LIMIT_ACTUAL': pl.Float64,
        'AMT_DRAWINGS_CURRENT': pl.Float64,
        'AMT_APPLICATION': pl.Float64,
        'DAYS_DECISION': pl.Float64,
        'DAYS_EMPLOYED': pl.Float64,
        'DAYS_BIRTH': pl.Float64,
        'DAYS_REGISTRATION': pl.Float64,
        'DAYS_ID_PUBLISH': pl.Float64,
        'MONTHS_BALANCE': pl.Float64,
        'DAYS_CREDIT': pl.Float64,
        'AMT_CREDIT_SUM': pl.Float64,
        'AMT_CREDIT_SUM_DEBT': pl.Float64,
        'AMT_CREDIT_SUM_LIMIT': pl.Float64,
        'AMT_CREDIT_SUM_OVERDUE': pl.Float64,
        'CNT_INSTALMENT': pl.Float64,
        'CNT_INSTALMENT_FUTURE': pl.Float64,
        'SK_DPD': pl.Float64,
        'SK_DPD_DEF': pl.Float64
    }
    ...
    test_base = pl.read_csv(raw_paths['application_test'], infer_schema_length=10000, null_values=["", "NA", "NaN", "nan", "NULL"])
    
    bureau = scan_safe(raw_paths['bureau'])
    bureau_balance = scan_safe(raw_paths['bureau_balance'])
    prev_app = scan_safe(raw_paths['previous_application'])
    pos_cash = scan_safe(raw_paths['pos_cash_balance'])
    installments = scan_safe(raw_paths['installments_payments'])
    cc_balance = scan_safe(raw_paths['credit_card_balance'])
...
def agg_installments(installments: pl.LazyFrame, config):
    """Step 3.4 - Installments Payments (Enhanced)"""
    print("Aggregating installments...")
    id_curr = config['training']['id_col']
    eps = config['globals']['division_epsilon']
    
    # Cast explicitly inside calculation for extreme safety
    installments = installments.with_columns([
        (pl.col('AMT_PAYMENT').cast(pl.Float64) / (pl.col('AMT_INSTALMENT').cast(pl.Float64) + eps)).alias('PAYMENT_PERC'),
        (pl.col('AMT_INSTALMENT').cast(pl.Float64) - pl.col('AMT_PAYMENT').cast(pl.Float64)).alias('PAYMENT_DIFF'),
        (pl.col('DAYS_ENTRY_PAYMENT').cast(pl.Float64) - pl.col('DAYS_INSTALMENT').cast(pl.Float64)).clip(lower_bound=0).alias('DPD'),
        (pl.col('DAYS_INSTALMENT').cast(pl.Float64) - pl.col('DAYS_ENTRY_PAYMENT').cast(pl.Float64)).clip(lower_bound=0).alias('DBD')
    ])
...
def agg_cc_balance(cc_balance: pl.LazyFrame, config):
    ...
    cc_balance = cc_balance.with_columns(
        (pl.col('AMT_BALANCE').cast(pl.Float64) / (pl.col('AMT_CREDIT_LIMIT_ACTUAL').cast(pl.Float64) + eps)).clip(0, 1).alias('UTILIZATION')
    )
...
def impute_missing(train: pl.DataFrame, test: pl.DataFrame, config):
    ...
    if test_aux_cols: test = test.with_columns([pl.col(c).fill_null(0) for c in test_aux_cols])
...
def add_global_features(df: pl.DataFrame, config):
    ...
    exprs = [
        (pl.col('AMT_CREDIT').cast(pl.Float64) / (pl.col('AMT_INCOME_TOTAL').cast(pl.Float64) + eps)).alias('CREDIT_INCOME_RATIO'),
        (pl.col('AMT_ANNUITY').cast(pl.Float64) / (pl.col('AMT_INCOME_TOTAL').cast(pl.Float64) + eps)).alias('ANNUITY_INCOME_RATIO'),
        (pl.col('AMT_ANNUITY').cast(pl.Float64) / (pl.col('AMT_CREDIT').cast(pl.Float64) + eps)).alias('CREDIT_TERM')
    ]
    
    ext_cols = fe_config['ext_sources']
    if all(c in df.columns for c in ext_cols):
        for c in ext_cols: df = df.with_columns(pl.col(c).cast(pl.Float64, strict=False))
    ...
    if all(c in df.columns for c in enq_cols):
        for c in enq_cols: df = df.with_columns(pl.col(c).cast(pl.Float64, strict=False))
        exprs.append(pl.sum_horizontal(enq_cols).alias('AMT_REQ_CREDIT_BUREAU_TOTAL'))

    if 'AMT_GOODS_PRICE' in df.columns:
        exprs.append((pl.col('AMT_GOODS_PRICE').cast(pl.Float64) / (pl.col('AMT_CREDIT').cast(pl.Float64) + eps)).alias('GOODS_CREDIT_RATIO'))
...
def feature_cleanup(train: pl.DataFrame, test: pl.DataFrame, config):
    ...
    for col in train.columns:
        if col not in [target_col, id_col]:
            ...
            if max_prop > var_threshold: to_drop_var.append(col)
    ...
    if missing_in_test: test = test.with_columns([pl.lit(0, dtype=pl.UInt8).alias(c) for c in missing_in_test])
