...
class TrainingConfigTest(unittest.TestCase):
    def test_training_defaults_match_balanced_profile_values(self):
        config = load_hydra_config()
        training = config["training"]

        self.assertEqual(training["cv_splits"], 3)
        self.assertEqual(training["optuna_n_trials"], 10)
        self.assertEqual(training["optuna_subsample_rate"], 0.35)
